...
**D11 — Redirects and the build command move to Phase 7, not Phase 5.** *(Phase 5)*
The plan put the netlify.toml 301s in Phase 5, but that file is what publishes
the site today. Editing it before the app is ready would touch production for no
benefit, so the redirects land with the build-command switch as one atomic
cutover commit, revertable in one step.

**D12 — Two things added that the bundle never had.** *(Phase 5)*
Both are shipped regardless of what we do, so the choice was only whether they
look right:
- **Favicon.** Generated by `build-icon.mjs` from the site's own moustache path
  on the preloader's red — not a new drawing. Checked at 16/32/64/96 px.
- **404 page.** Next emits `404.html` whether or not we write one, and its
  default is an unstyled white page that would read as broken here. Ours carries
  the same chrome and type as every route. The bundled site had none — Netlify
  served its own — so this is an addition, and it is deliberately kept out of the
  35-shot matrix, which means "identical to the old site".
